...
- E2E inyecta `VITE_API_BASE_URL=http://127.0.0.1:3100` y abre React/Vue en `http://127.0.0.1:5174` / `:5175` (mismo host que la API — las cookies no cruzan bien entre `localhost` y `127.0.0.1`). Vanilla sigue en `localhost`. Los proyectos **visual** React/Vue usan los mismos `baseURL` que los E2E funcionales.

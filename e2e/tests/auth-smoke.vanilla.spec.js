...
const { runAuthSmokeFlow, runOAuthMockSmokeFlow } = require('../helpers/auth-smoke-flow.js');
...
test.describe('Smoke auth — dashboard vanilla', () => {
  ...
  test('gate → oauth mock → tabla con datos → logout → gate', async ({ page }) => {
    await page.goto('/');
    await runOAuthMockSmokeFlow(page);
  });
});

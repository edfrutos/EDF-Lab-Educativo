...
2. Construye la imagen Docker:
   ...
   ```
   Observa el output: Docker descarga la imagen base `node:22-alpine`, instala las dependencias con `npm ci` y copia el código. El último mensaje debe ser algo como `naming to docker.io/library/edf-lab-api`.
...
6. Comprueba los logs del contenedor para ver la actividad:
   ```bash
   docker logs edf-lab-api
   ```

7. Para el contenedor (Ctrl+C en la terminal donde está corriendo, o en otra terminal):
   ```bash
   docker stop edf-lab-api
   ```

8. Vuelve a arrancar el contenedor y comprueba si el usuario del paso 5 persiste:
   ```bash
   npm run docker:start
   ```
   En otra terminal:
...
En el paso 8, "Docker User" ya no aparece en la lista de usuarios. El contenedor arranca con los dos usuarios del SEED_DATA (`John Doe` y `Jane Smith`). Los datos creados en la sesión anterior se perdieron porque el contenedor es efímero: `data/users.json` no se copia dentro de la imagen.

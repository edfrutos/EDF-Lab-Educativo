...
Tests: `npm run test:db:prepare` (desde la raíz) y luego `npm test` — **71 tests** si Postgres está en marcha (36 en SQLite + 35 en Postgres). Solo SQLite: `npm run test:sqlite --prefix api` (**36 tests**). **CI:** cada push o PR a `main` ejecuta cuatro jobs en paralelo (`test-sqlite`, `test-postgres`, `e2e-smoke` con Chromium+Firefox, `e2e-postgres`) — ver [`docs/10-tests.md`](./docs/10-tests.md#ci-en-github-actions).
